...
const DirectionFlags DIR_NONE = 0;
const DirectionFlags DIR_UP = 1;      // U
const DirectionFlags DIR_RIGHT = 2;   // R
const DirectionFlags DIR_DOWN = 4;    // D
const DirectionFlags DIR_LEFT = 8;    // L
const DirectionFlags DIR_ALL = 15;    // All directions (U+R+D+L)

//An enum for all the Element types.
enum ElementTypes { EMPTY, SOLID, LIQUID, POWDER, HEAT, GAS, STONE };

enum ReactionTypes { NORMAL, EXPLOSION, PIPE, TELEPORT,SQUARE,CLEAR};
...
public:
	...
	DirectionFlags directions[MAX_ELEMENTS];
	int amt = 0;
	ReactionTypes type = NORMAL;

	Reaction() {
		for (int i = 0; i < MAX_ELEMENTS; i++) {
			directions[i] = DIR_ALL; // Default to ALL directions
		}
	}
};
...
private:
	...
	void add_reaction(std::string elea, std::string eleb, std::string result_a, std::string result_b, double chance,
		ReactionTypes type = NORMAL, bool all = false, DirectionFlags dirs = DIR_ALL);
	void init_elements();
